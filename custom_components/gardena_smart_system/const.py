...
# Gardena service states that represent an error/warning condition.
MOWER_ERROR_STATES: Final = frozenset({
    MOWER_STATE_ERROR,
    MOWER_STATE_WARNING,
})
...
# RFC-6455 WebSocket ping interval (seconds).
# Gardena API documentation recommends a ping every 150 seconds.
WEBSOCKET_PING_INTERVAL: Final = 150

# How often to check whether the current WebSocket session should be renewed.
WEBSOCKET_SESSION_CHECK_INTERVAL: Final = 60

# Proactive reconnect before the 2-hour WebSocket session limit.
# 119 minutes = 7140 s, leaving approximately one minute of margin.
